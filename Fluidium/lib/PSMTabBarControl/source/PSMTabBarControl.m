...
- (id)initWithFrame:(NSRect)frame
{
    ...
    if (self) {
        ...
        // BEGIN FLUIDIUM
        NSMutableArray *types = [NSMutableArray arrayWithObjects:@"PSMTabBarControlItemPBType", WebURLsWithTitlesPboardType, NSURLPboardType, nil];
        // END FLUIDIUM
...
- (BOOL)performDragOperation:(id <NSDraggingInfo>)sender
{
    ...
    PSMTabBarCell *cell = [self cellForPoint:p cellFrame:NULL];
    NSInteger index = [_cells indexOfObject:cell];    
    BOOL droppedOnCell = NSNotFound != index;
    // END FLUIDIUM
    ...
    // BEGIN FLUIDIUM
	} else if (!droppedOnCell && (hasWebURLs || hasURLs)) {
...
- (void)tabView:(NSTabView *)aTabView didSelectTabViewItem:(NSTabViewItem *)tabViewItem
{
    ...
    if ([_cells count] > 0 && tabIndex < [_cells count]) {
		...
		if (_alwaysShowActiveTab && [thisCell isInOverflowMenu]) {
            ...
            // BEGIN FLUIDIUM
			[[tabViewItem retain] autorelease];
			[[thisCell retain] autorelease];
            // END FLUIDIUM
